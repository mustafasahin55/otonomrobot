import Bitirme_pathfinding.HybridAstarPlanner.astar as astar
import Bitirme_pathfinding.HybridAstarPlanner.draw as draw
import Bitirme_pathfinding.HybridAstarPlanner.reeds_shepp as rs
...
class C:  # Parameter config
    ...
    RF = 40  # [m] distance from rear to vehicle front end of vehicle
    RB = 10  # [m] distance from rear to vehicle back end of vehicle
    W = 32  # [m] width of vehicle
    WD = 0.5 * W  # [m] distance between left-right wheels
    WB = 32  # [m] Wheel base
    TR = 10  # [m] Tyre radius
    TW = 5  # [m] Tyre width
...
def design_obstacles(x, y, indices_np,newx,newy):
    ox, oy = [], []
    for i in range(newx-10,newx+10,1):
        for j in range(newy-10,newy+10,1):
            ox.append(i)
            oy.append(j)
    for i in range(x):
        ox.append(i)
...
def call_Path(current_x,current_y,current_yaw,x_goal,y_goal,yaw_goal,newx,newy):
    print("start!")
    
    with open("Functions/Bitirme_pathfinding/HybridAstarPlanner/sonuc_matrisi.txt", "r") as dosya:
        satirlar = dosya.readlines()
    ...
    indices_np = np.argwhere(matris == 1)

    x, y = int(matris.shape[0]/2), int(matris.shape[1]/2)
    
    sx, sy, syaw0 = current_x, current_y, np.deg2rad(current_yaw)
    gx, gy, gyaw0 = x_goal, y_goal, np.deg2rad(yaw_goal)

    ox, oy = design_obstacles(x, y, indices_np/2,newx,newy)
...
def main():
    x,y,yaw,direction = call_Path(150,217,70,250,350,0,190,290)
