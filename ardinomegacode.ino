...
//////Değişkenler/////////////////////////
//USB Serial/////////////////////////////
int serialBaud = 31250;
const byte numChars = 32;
char receivedChars[numChars];
char tempChars[numChars];
int integerFromPC = 0;
int integersFromPC[5];
...
#define LEFT -1
#define RIGHT 1
uint8_t motorPins[] = { 24, 25 };
uint8_t motorPWMPins[] = { 4, 5, 2, 7, 6, 3 };
const float a = 0.160;
const float b = 0.160;
const float B = 0.245;
float VL1, VL2, VL3, VR1, VR2, VR3;
float VL[] = { VL1, VL2, VL3, VR1, VR2, VR3 };
float R;
/////////////////////////////////////////
//MPU6050
int yaw_1 = 0;
...
//AcX = 377 | AcY = -31 | AcZ = -347 | GyX = -47 | GyY = 67 | GyZ = -12
// Transform raw data of accelerometer & gyroscope
#define MPU6050_AXOFFSET 377
#define MPU6050_AYOFFSET -31
#define MPU6050_AZOFFSET -347
...
#define MPU6050_GXOFFSET -47
#define MPU6050_GYOFFSET 67
...
const int sRMPin = 9;
/////////////////////////////////////////
//////Fonksiyon Headerları///////////////
//void initSerial();   //USB Seriali açar.
//void readMPU6050();  //MPU6050 verilerini okur.
//void initMPU(); //MPU6050 hazırlar.
//void leftTurnServo(double targetAngle,int angle)
//void rightTurnServo(double targetAngle,int angle)
// void sendNode()
//void dcMotorControl(int V, int angle, int direction)
/////////////////////////////////////////
...
void loop() {
  ...
  if (data.fonk == 1) {
    ...
    turn(data.targetA, data.servoA, data.velo, data.dir, data.rot);
    data.fonk = 2;
    data.targetA=0; 
    data.servoA=0; 
    data.velo=0; 
...
///////////Servo Baslangic/////////////////
void initServos() {
  servoLF.attach(sLFPin);
  servoRF.attach(sRFPin);
  servoLB.attach(sLBPin);
  servoRB.attach(sRBPin);
  servoLM.attach(sLMPin);
  servoRM.attach(sRMPin);
  setServoDefault();
}
void initDCMotors() {
...
int yaw_N = 0;
void turn(double targetAngle, int servoAngle, int V, int direction, int rotation) {
  ////Serial.println(servoAngle);
  //Serial.print("targetA:");//Serial.print(targetAngle);//Serial.print("servoA:");//Serial.print(servoAngle);//Serial.print("V:");//Serial.print(V);
  //Serial.print("Dir:");//Serial.print(direction);//Serial.print("rot:");//Serial.print(rotation);//Serial.println();
        ...
        case (-1):
          if (servoAngle >0){
          setServoLeft(servoAngle);}else{
            setServoDefault();
          }
          //Serial.println("swCase-1");
          break;

        case 1:
        //Serial.println("swCase1");
          if (servoAngle >0){
          setServoRight(servoAngle);}else{
            setServoDefault();
          }
          break;
        case 2:
          //Serial.println("swCase2");
          setServoDefault();
          break;
      }


    dcMotorControl(V, targetAngle, direction, rotation);
...
void setServoDefault() {
  servoLF.write(LFAngle);
  servoRF.write(RFAngle);
  servoLB.write(LBAngle);
  servoRB.write(RBAngle);
  servoLM.write(LMAngle);
  servoRM.write(RMAngle);
}

void setServoLeft(int angle) {
  ////Serial.println("servoleft");
  servoLF.write(LFAngle - angle);
  servoRF.write(RFAngle - angle);
  servoLB.write(LBAngle + angle);
  servoRB.write(RBAngle + angle);
}
void setServoRight(int angle) {
...
void initSerial() {
  ...
  ////Serial.print("Serial is open. Baud:");
...
void parseData(String str) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    String valStr = str.substring(index, nextIndex);
    int val = valStr.toInt();
    switch (i) {
      case 0: data.fonk = val; break;
      case 1: data.targetA = val; break;
      case 2: data.servoA = val; break;
      case 3: data.velo = val; break;
      case 4: data.dir = val; break;
      case 5: data.rot = val; break;
    }
    index = nextIndex + 1;
  }
}
...
void mpu6050_GetData() {
  //Serial.println("mpu6050_GetData");
...
void mpu6050_getRollPitchYaw() {
  //Serial.println("mpu6050_getRollPitchYaw");
...
void dcMotorControl(int V, int angle, int direction, int rotation) {
//Serial.println("dckontrol");

  if ((angle > 0)) {

    //Serial.print("angle!=0");Serial.println();

    R = a / tan(angle);

    VL1 = (V * sqrt((sq(a)) + sq((R + 0.5 * B)))) / R;  // 4
    VR1 = (V * sqrt((sq(a)) + sq((R - 0.5 * B)))) / R;  // 7

    VL2 = (V * (R + 0.5 * B)) / R;  // 5
    VR2 = (V * (R - 0.5 * B)) / R;  // 6

    VL3 = (V * sqrt((sq(b)) + sq((R + 0.5 * B)))) / R;  // 2
    VR3 = (V * sqrt((sq(b)) + sq((R - 0.5 * B)))) / R;  // 3

    float max_velocity = max(max(max(VL1, VR1), max(VL2, VR2)), max(VL3, VR3));
    float scaling_factor = 255 / max_velocity;
    ...
    //Serial.print("angle!=0-else");Serial.println();
    VL1 = V;
    VL2 = V;
    VL3 = V;
    VR1 = V;
    VR2 = V;
    VR3 = V;
    //Serial.print("VL1: ");Serial.println(VL1);
  }

  switch (direction) {
    case 0:
      //Serial.print("direction: ");Serial.println("0");
    ...
    case 1:
      //Serial.print("direction: ");Serial.println("1");
      digitalWrite(motorPins[0], 1);
      digitalWrite(motorPins[1], 0);
      //Serial.println("motorpinss");

      setDCSpeed(rotation);
      break;

    case 2:
      //Serial.print("direction: ");Serial.println("2");
...
void setDCSpeed(int rotation) {
//Serial.println(rotation);
  switch (rotation) {
    case 1:
      //Serial.print("setDCSpeed ");Serial.println("1");
      analogWrite(motorPWMPins[0], VL1);
      analogWrite(motorPWMPins[1], VL2);
      analogWrite(motorPWMPins[2], VL3);
      analogWrite(motorPWMPins[3], VR1);
      analogWrite(motorPWMPins[4], VR2);
      analogWrite(motorPWMPins[5], VR3);
      break;
    case -1:
      //Serial.print("setDCSpeed ");Serial.println("-1");
      analogWrite(motorPWMPins[0], VR1);
      analogWrite(motorPWMPins[1], VR2);
      analogWrite(motorPWMPins[2], VR3);
      analogWrite(motorPWMPins[3], VL1);
      analogWrite(motorPWMPins[4], VL2);
      analogWrite(motorPWMPins[5], VL3);


      break;
    case 2:
      //Serial.print("setDCSpeed ");Serial.println("2");
      analogWrite(motorPWMPins[0], VL1);
      analogWrite(motorPWMPins[1], VL2);
      analogWrite(motorPWMPins[2], VL3);
      analogWrite(motorPWMPins[3], VR1);
